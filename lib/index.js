	...
	ec2.describeImages(
		{
			Filters: [
				{
					Name: "name",
					Values: [config.AWSInstanceType === "g2.2xlarge" ? "g2*" : "g3*"]
				}
			],
			Owners: ["589318761596"],
		},
...
function start(cb) {
	...
	var _optimizeEphemeralUsage = function(cb) {
		if (isCancelled) {
			cb(cancelledError);
			return;
		}

		reporter.report("Optimizing Ephemeral Disk Usage");
		sendMessage(getPS1("Optimize-Ephemeral-Disk-Usage.ps1"), cb);
	}
		...
		getAMI(function(err, ami) {
			...
			if (ami) {
				...
				startFunctions = [
					...
					_optimizeEphemeralUsage,				// Optimize-Ephemeral-Disk.ps1
